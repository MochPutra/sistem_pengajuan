...
$statement = $pdo->prepare('SELECT * FROM pengajuan WHERE id = ? LIMIT 1');
$statement->execute([$id]);
$pengajuan = $statement->fetch();
...
if (!isAdmin() && $pengajuan['user_id'] !== $_SESSION['user']['id']) {
    flash('Anda tidak memiliki izin untuk mengedit pengajuan ini.', 'danger');
    header('Location: pengajuan.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $update = $pdo->prepare('UPDATE pengajuan SET title = ?, description = ?, signature_data = ? WHERE id = ?');
        $update->execute([$title, $description, $signature, $id]);
        ...
        if (!empty($_FILES['attachments']['name'][0])) {
            ...
            foreach ($_FILES['attachments']['error'] as $index => $error) {
                ...
                if (move_uploaded_file($tmpName, $destination)) {
                    $stmtFile->execute([$id, $originalName, 'uploads/attachments/' . $newName, $mimeType]);
                }
            }
        }
...
$attachmentsStmt = $pdo->prepare('SELECT * FROM pengajuan_files WHERE pengajuan_id = ?');
...
<div class="row justify-content-center">
    <div class="col-lg-10">
        <div class="card shadow-sm border-0">
            <div class="card-body">
                <h4 class="card-title mb-4">Edit Pengajuan</h4>
                <?php if ($errors): ?>
                ...
                <form method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">Unggah file tambahan jika diperlukan.</div>
                    </div>
                    <?php if ($attachments): ?>
                        <div class="mb-3">
                            ...
                            <ul class="list-group">
                                <?php foreach ($attachments as $file): ?>
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <span><i class="bi bi-file-earmark-text"></i> <?= escape($file['filename']); ?></span>
                                        <a href="<?= escape($file['filepath']); ?>" target="_blank" class="btn btn-sm btn-outline-secondary">Lihat</a>
                                    </li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
